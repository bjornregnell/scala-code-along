...
ourList = "rainbow" +: ourList 
if (ourList(0) == ourList(1) && ourList(0) == "rainbow") println("Two rainbows")
...
By adding 'case' in front of 'class' you get all these goodies for free generated by the compiler:
...
*/
val o1 = Orange(123)
Orange(123) == Orange(123) //structural equality using the compiler generated case class == method
Vector(Orange(42), Orange(43)).collect{case Orange(g) => g + 42} //pattern matching by the generated unapply method
Vector(Orange(42), Orange(43)).collect{case Orange(g) if g > 42 => g + 42} //pattern matching with guard
...
//functions can be implicit, causing the compiler to implicitly invoke a conversion if needed
inc("42") // error: type mismatch
implicit def fromStringToInt(s: String): Int = s.size
inc("42") // now the compiler will not give up as easy as it finds an implicit conversion from String to Int

//functions can be "curried" with multiple argument lists
def plus(a: Int)(b: Int) = a + b
val inc3 = plus(1) _  //partially applied 

//functions can have by name parameters, using the syntax : =>
def sideEffect(a: Int) = {println(s"side effect $a"); a}  
def byValeAddSelf(i: Int) = i + i
def byNameAddSelf(i: => Int) = i + i 
byValeAddSelf(sideEffect(42))
byNameAddSelf(sideEffect(42))

//functions that only have one parameter can take block directly
inc{42} // expanded to equivalent inc({42}) by the compiler

//making your own loop-function looking like a part of the language
def loop(n: Int)(block: => Unit) = {var i = 0; while (i < n) {block; i += 1} }
loop(42){
  print("Na")
}